...
int do_start(int syscall, int pid, int status) {
	...
	//printf("status_start=%d, other_start=%d\n", status, vsyscall_arg(MY_CUSTOM_SYSCALL, 3, REQUEST_START_MONITORING, syscall, pid));
	//printf("pid_start=%d\n", pid);
	return 0;
}
...
int do_stop(int syscall, int pid, int status) {
	...
	//printf("status_stop=%d, other_stop=%d\n", status, vsyscall_arg(MY_CUSTOM_SYSCALL, 3, REQUEST_STOP_MONITORING, syscall, pid));
	//printf("pid_stop=%d\n", pid);
	return 0;
}
