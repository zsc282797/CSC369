...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		pgtbl_entry_t* victim_pg = coremap[frame].pte;
		//victim page from the Physical memory 
		
		
		//Mask out the dirty bit 
		if (!((victim_pg->frame) & PG_DIRTY)){ //Swaping a clean page 
			 
			//Set the victime page to be INVALID
			evict_clean_count++; //increment the counter 
		}
		else { 
			victim_pg -> frame = victim_pg -> frame | PG_ONSWAP;
           	evict_dirty_count ++; //increment the counter 

		}
		
		int swap_offset = swap_pageout(frame, victim_pg->swap_off);
		if (swap_offset == INVALID_SWAP){
			perror("Invalid Swap \n");
			exit(1);
		} 
		victim_pg->swap_off = swap_offset;



           victim_pg->frame = victim_pg->frame & (~PG_VALID); 
           victim_pg->frame = victim_pg->frame & (~PG_DIRTY); 
	}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	}//necessary initialization of second level tables 
	
	// Use top-level page directory to get pointer to 2nd-level page table
	
	pgtbl_entry_t* sec_page_table = (pgtbl_entry_t*)(pgdir[idx].pde & PAGE_MASK);
	
	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	p = sec_page_table + PGTBL_INDEX(vaddr);

	// Check if p is valid or not, on swap or not, and handle appropriately
	if (p->frame & PG_VALID){ //if the page is valid 
		
		hit_count ++; //Nothing to do since the page is valid 
	}
	else { //if the page is invalid 
		if (p->frame & PG_ONSWAP){ //if page is onswap
			int new_frame = allocate_frame(p);
			if ((swap_pagein(new_frame,p->swap_off))!=0){
				perror("Failed Swap in\n");
				exit(1);
			}
			p->frame = new_frame << PAGE_SHIFT; //give space for status
			p->frame = p->frame | PG_ONSWAP;
			p->frame = p->frame & (~PG_DIRTY);

			coremap[new_frame].vaddr = vaddr;
			coremap[new_frame].vaddr &= ~0xF; //Storing for later compare in OPT
		}
		else {//if the page is not onswap
			int new_frame = allocate_frame(p);
	...
	p->frame = p->frame | PG_VALID; //set p to be valid 
	p->frame = p->frame | PG_REF; //set p to be referenced 

	if (type == 'M' || type == 'S'){ //if will be written to 
		p->frame = p->frame | PG_DIRTY; //set the page to be dirty 
	} 
