...
void my_exit_group(int status)
{
    ...
    //Lock the table for synchronization
    del_pid(current->pid);
    //Delete the pid from the monitored list
    spin_unlock(&my_table_lock);
    
    //Call the original exit group
...
asmlinkage long interceptor(struct pt_regs reg) {
    //reg.ax stores the requested system call number when trap 
    spin_lock(&my_table_lock);
    //Lock the table for sychronized access
    

    if (table[reg.ax].monitored == 1 ){//if the system call is monitored for at least one pid
        
        if(check_pid_monitored(reg.ax, current->pid) ){ // if the pid is being in my_list
            log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
            //The log message macro
            //if pids are on the black list, monitor
        }
    }

    else if (table[reg.ax].monitored == 2){//if all the system call is monitored for all pids
        //check if the pid is on the white list 
        if ( !check_pid_monitored(reg.ax, current->pid) ){  
                //if the pids are not on the white list, we log 
        log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp); }
        // To log if the pid is not on the black list a.k.a not stopped being monitored 
    }

    spin_unlock(&my_table_lock);


	return table[reg.ax].f(reg);//call the original requested system call
    // Just a placeholder, so it compiles with no warnings!
}
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    //Check the cmd to be one of the allowed ones
    
    
    
    //check the reuqesed syscall paramter to be valid and legal
    ...
    if ( cmd == REQUEST_SYSCALL_INTERCEPT ) {//in the Request to hijack a system_call
        //Check the user if is root 
        if (current_uid() != 0){
            return -EPERM;
        }
        ...
        //Lock the tables for synchronization
        
        if ( table[syscall].intercepted == 1 ){ //if the specified syscall is already intercepted
             spin_unlock(&my_table_lock);
        ...
        table[syscall].intercepted = 1; //raised the flag as the call is intercepted
        table[syscall].f = sys_call_table[syscall]; //initialize the callable system function
       
    
        spin_unlock(&my_table_lock);
        
        set_addr_rw( (unsigned long)sys_call_table );
        //set the system_call table to be writable
        
        sys_call_table[syscall] = interceptor;
        //Insert our interceptor to the sys_call_table
    ...
    else if ( cmd == REQUEST_SYSCALL_RELEASE ){// in the Request of Deregister a system call
        ...
        //spin_lock(&sys_call_table_lock);
        //Lock the tables for synchronization
        if (table[syscall].intercepted == 0 ){ //if the specified syscall is not intercepted
            spin_unlock(&my_table_lock);
            //spin_unlock(&sys_call_table_lock);
            return -EINVAL;
        }
        ...
        destroy_list(syscall); //destroy the linked list of its pids
        sys_call_table[syscall] = table[syscall].f; // put the original list to invoke
    ...
    else if ( cmd == REQUEST_START_MONITORING) { // in the Request of start monitoring call
        
        //Check the pid if it is valid and belong to a task
         if ( (pid < 0)){
        ...
        if (current_uid() != 0){ //if user is not root
            if (pid == 0) { return -EPERM; } //Do not allow pid = 0
            else {  //we check that the calling pid owns the requested pid
        ...
        if (table[syscall].monitored == 0) { //empty whilte list 
            if (pid == 0) { 
                            ...
                            return 0; 
                            }  //enter white list mode
            else if (pid > 0){
                if (add_pid_sysc(pid, syscall)!=0) { 
                    spin_unlock(&my_table_lock);
                    return -ENOMEM; }
                else {table[syscall].monitored = 1;
                    spin_unlock(&my_table_lock);
                    return 0; 
                }
            }
        ...
        if (table[syscall].monitored == 1) { // black list mode , on the list 
            if (pid == 0) { 
                            destroy_list(syscall);
                            INIT_LIST_HEAD( &(table[syscall].my_list) );
                            table[syscall].monitored = 2; 
                ...
                if ( check_pid_monitored (syscall, pid)!=0 ) { 
                    ...
                    spin_unlock(&my_table_lock);
                    return -ENOMEM; }
                else {spin_unlock(&my_table_lock);
                        return 0; }
            }
        }

        
        if (table[syscall].monitored == 2) { //White list mode, on the list = not to monitor
            if (pid == 0) { destroy_list(syscall);
                            INIT_LIST_HEAD(& (table[syscall].my_list));
                            spin_unlock(&my_table_lock);
                            return 0; } // delete everythin on the whilte list
            if (pid > 0) { 
                    if (check_pid_monitored(syscall,pid)) {// if the pid is on the list, not logged 
                        del_pid_sysc(pid, syscall);
                        spin_unlock(&my_table_lock);
                        return 0;
                         //remove it from the list in order to log
                    }
                    else { // if the pid is not on the list, meaning arealdy logged
                        spin_unlock(&my_table_lock);
                        return -EBUSY;
                    }
            }
        ...
        //spin_lock(&sys_call_table_lock);
        //Lock the tables for synchronization
        //Test1: test the syscall is intercepted
        //Check the pid if it is valid and belong to a task
        if ( (pid < 0)){
            spin_unlock(&my_table_lock);
            return -EINVAL; //for DEBUG!
        ...
        if (current_uid ()!= 0){ //if user is not root
            if (pid == 0) { 
                spin_unlock(&my_table_lock);
                return -EPERM; } //Do not allow pid = 0
            else {  //we check that the calling pid owns the requested pid
                if ( check_pids_same_owner(current->pid, pid)!=0) {
                    spin_unlock(&my_table_lock);
                    return -EPERM;
        ...
        if (table[syscall].monitored == 1){  //Black list, on the list = log 
            if (pid == 0){ //stop everything 
            ...
            else if ( check_pid_monitored(syscall,pid)!=0 ) {//Chekc if is on the list
                    del_pid_sysc(pid,syscall);
                    spin_unlock(&my_table_lock);
                    return 0;    
        ...
        if (table[syscall].monitored == 2){ //White list, on the list = not log 
            if (pid == 0){
                destroy_list(syscall);
                INIT_LIST_HEAD( &(table[syscall].my_list) );
                table[syscall].monitored = 0;
                //table[syscall].monitored = 0;
                printk(KERN_DEBUG "We got pid == 0");

                spin_unlock(&my_table_lock);
                return 0;
            }
            else { //pid > 0 
                if (check_pid_monitored(syscall, pid)==1){// if it is on the list, not logged  
                    spin_unlock(&my_table_lock);
                    
                    return -EINVAL;
                    //DEBUG!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                }
                else { // if it is not on the list, being loogged
                    if ( add_pid_sysc(pid,syscall)!=0 ){ 
                        spin_unlock(&my_table_lock);
                        return -ENOMEM;}  
                }
...
static int init_function(void) {
    int i ; // defined for the for loop later on 
    //prepare the lock and set the calltable to be writable
    //printk(KERN_DEBUG "Loading Kernel!!!!\n");
    spin_lock(&sys_call_table_lock);
    set_addr_rw((unsigned long)sys_call_table);
    //We need to preserve the old call which is taken place by our new system call
    
    orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
    
    //Also need to save the exit_group
    ...
    //for safety we need to initialize our table
    spin_lock(&my_table_lock);
...
{   int j ; //defined for the loop 
    //RELEASE ALL THE SYSCALL
    spin_lock(&my_table_lock);
    
    for (j=0; j< NR_syscalls+1;j++){
        if (table[j].intercepted){
        my_syscall(REQUEST_SYSCALL_RELEASE,j,1); //use our own SYSCALL RELEASE
        }
    }
    
    spin_unlock(&my_table_lock);
    
    //Restore the original function and the original exit group
