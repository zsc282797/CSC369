...
int main(int argc, char **argv) {
	...
	int inode_used = 0;
 	...
 	if (argc != 3) {
  	...
  	char * p_path = malloc(sizeof(char)* (strlen(argv[2])+4) );//parent path 
  	strncpy(path, argv[2],strlen(argv[2])); //copy over the path
    ...
    path = new_path;
  	...
  	int free_block = free_blocks[0];
  	struct ext2_dir_entry * new_dir = (struct ext2_dir_entry*) (disk + EXT2_BLOCK_SIZE * (free_block+1));
  	//Format the newly allocated free block to a new dirent.
  	printf("First allocation of dirent blocks: %d\n",free_block+1);
  	...
  	int old_size = padded_size; //store the old size 
	//Move on to the parent .. 
  	new_dir = (void*)new_dir + padded_size;
  	strcpy(new_dir->name,"..");
  	new_dir -> name_len = 2;
  	new_dir-> file_type = EXT2_FT_DIR; //set type to be normal directory 
  	new_dir -> inode = pinode_idx + 1; //set the inode for the dir_ent
	size = sizeof(struct ext2_dir_entry) + new_dir-> name_len;
  	...
  	new_dir -> rec_len = EXT2_BLOCK_SIZE - old_size; //We want to exahst the block 


  	mask_block_bit(free_block); block_used++;
  	/*----------------------- Set up the inode data ------------ */

  	struct ext2_inode * dir_inode = (struct ext2_inode *)(disk + gc->bg_inode_table * 1024+ (free_inode_idx) * sb->s_inode_size);
	...
	if (!found){//if no such space exist in the blocks table 
		...
		dir_ent =(struct ext2_dir_entry *)(disk + (free_blk[0]) *EXT2_BLOCK_SIZE);
